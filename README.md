# Yuhan Shu

Undergraduate at **Nanjing University**. I work across **AI agents, world models, graphics, and quantitative research**.

I like turning research ideas into small, testable systems - with clear assumptions, reproducible experiments, and honest limitations.

## Current interests

- **AI agents & mathematical reasoning** - solver-verifier workflows, tool use, evaluation, and multi-agent coordination
- **World models** - reading and organizing recent work on learned environment models and structured prediction
- **Graphics & 3D algorithms** - reconstruction, UV-to-3D localization, point-cloud geometry, and navigation
- **Quantitative research** - independent factor research, validation, and research-oriented backtesting

## Selected projects

| Project | What it shows |
| --- | --- |
| [RealityScan UV-to-3D Navigation](https://github.com/Yu-han-Shu/realityscan-uv3d-navigation) | 3D localization and collision-aware point-cloud path planning |
| [MathAgent Demo](https://github.com/Yu-han-Shu/mathagent_demo) | Solver-verifier workflow, local symbolic tools, batch experiments, and reports |
| [Quant Factor Research Lab](https://github.com/Yu-han-Shu/quant-factor-research-lab) | Modular factor evaluation, validation, and cost-aware backtesting |
| [New-Energy Strategic Radar](https://github.com/Yu-han-Shu/new-energy-strategic-radar) | TypeScript data product with de-identified sample data |
| [Synera Auto Arena](https://github.com/Yu-han-Shu/PA-Synera-Synergy-Auto-Arena) | C++/Qt object-oriented game prototype with state-machine logic |
| [Resume Interview Assistant](https://github.com/Yu-han-Shu/resume-interview-assistant) | Lightweight LLM application for resume parsing and interview practice |

## Toolbox

**Languages:** Python, C/C++, TypeScript, MATLAB  
**Research & libraries:** NumPy, pandas, SciPy, scikit-learn, SymPy, Open3D, COLMAP  
**Practice:** Git/GitHub, testing, experiment tracking, LaTeX

